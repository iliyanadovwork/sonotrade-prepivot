...
export function processMultiChartData(
  ...
  const step = getFixedStep(adjustedInterval, rangeSec, isMobile);
  ...
  for (let i = 0; i < sortedTimestamps.length; i++) {
    ...
    // This is needed for step charts to maintain flat lines
...
function getFixedStep(interval: string, rangeSec: number, isMobile: boolean = false): number {
  // Mobile gets less granular data to prevent blinking/performance issues
  const mobileMultiplier = isMobile ? 2 : 1;

  switch (interval) {
case "1h": return (isMobile ? 5 : 2) * 60; // Mobile: 5min, Desktop: 2min
case "6h": return (isMobile ? 20 : 10) * 60; // Mobile: 20min, Desktop: 10min
case "1d": return (isMobile ? 60 : 30) * 60; // Mobile: 1hr, Desktop: 30min
case "1w": return (isMobile ? 4 : 2) * 60 * 60; // Mobile: 4hr, Desktop: 2hr
case "1m": return (isMobile ? 12 : 6) * 60 * 60; // Mobile: 12hr, Desktop: 6hr
    case "all":
      if (rangeSec <= 60 * 60) return (isMobile ? 5 : 2) * 60; // Mobile: 5min, Desktop: 2min
      if (rangeSec <= 6 * 60 * 60) return (isMobile ? 20 : 10) * 60; // Mobile: 20min, Desktop: 10min
      if (rangeSec <= 24 * 60 * 60) return (isMobile ? 60 : 30) * 60; // Mobile: 1hr, Desktop: 30min
      if (rangeSec <= 7 * 24 * 60 * 60) return (isMobile ? 4 : 2) * 60 * 60; // Mobile: 4hr, Desktop: 2hr
      if (rangeSec <= 30 * 24 * 60 * 60) return (isMobile ? 12 : 6) * 60 * 60; // Mobile: 12hr, Desktop: 6hr
      if (rangeSec <= 3 * 30 * 24 * 60 * 60) return (isMobile ? 24 : 12) * 60 * 60; // Mobile: 24hr, Desktop: 12hr
...
export function processSingleChartData(
  ...
  const rangeSec = now - startTime;
  const formattingInterval = getFormattingInterval(rangeSec);
  const step = getFixedStep(adjustedInterval, rangeSec, isMobile); // reuse same step logic as multi-chart
  ...
  for (let i = 0; i < realDataPoints.length; i++) {
    ...
    // Insert forward-filled points for step chart behavior (flat lines)
    // For long time ranges (>30 days), limit forward-fill to avoid performance issues and confusion
    if (nextPoint) {
      const gap = nextPoint.timestamp - point.timestamp;
      const isLongTimeRange = rangeSec > 30 * 24 * 60 * 60; // More than 30 days
      
      // For long time ranges with large gaps (e.g., daily candles), only add minimal forward-fill
      if (isLongTimeRange && gap > 12 * 60 * 60) { // Gap larger than 12 hours
        // Only add one point just before the next candle to maintain step appearance
        const midPoint = nextPoint.timestamp - 60; // 1 minute before next point
        if (midPoint > point.timestamp) {
          pushPoint(midPoint, point.value);
        }
      } else if (gap > step * 1.5) {
        // For shorter time ranges, use normal forward-fill logic
        let inserted = 0;
        const maxInserts = isLongTimeRange ? 10 : 1500; // Limit inserts for long ranges
